...
const getTestDrives1 = async (req, res) => {
  try {
    // Fetch all test drive records
    const testDrives = await TestDrive.find({});
    
    // Respond with the retrieved data
    res.status(200).json(testDrives);
  } catch (error) {
    // Handle errors and respond with a 404 status
    res.status(404).json({ message: error.message });
  }
};
...
module.exports = {createTestDrive, getTestDrives,getTestDriveById, updateTestDrive, cancelTestDrive, deleteTestDrive,getTestDrives1  }
