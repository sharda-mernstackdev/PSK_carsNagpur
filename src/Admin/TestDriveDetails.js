...
const TestDriveDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3000/api/testDrive/get1/${id}`, {
          method: "GET",
        //   headers: {
  ...
  const handleCancel = async () => {
    try {
      const response = await fetch(`http://localhost:3000/api/testDrive/${id}/cancel1`, {
        method: 'PATCH',
        // headers: {
        //   Authorization: `Bearer ${localStorage.getItem('token')}`,
        //   'Content-Type': 'application/json',
  ...
  const confirmDelete =async () => {
    try {
      const response = await fetch(`http://localhost:3000/api/testDrive/delete1/${id}`, {
        method: 'DELETE',
        // headers: {
        //   Authorization: `Bearer ${localStorage.getItem('token')}`,
        //   'Content-Type': 'application/json',
