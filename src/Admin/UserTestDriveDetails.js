...
export default function UserTestDrive() {
  ...
  const fetchCarsData = async () => {
    try {
      ...
      const response = await fetch(
        "http://13.201.129.59:3000/api/testdrive/get1",
        {}
      );
      if (!response.ok)
  ...
  const fetchCarById = async (carId) => {
    ...
    const url = `http://13.201.129.59:3000/api/cars/cars/${carIdString}`;
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      carDataResults.forEach((car, index) => {
        if (car && testDrives[index].carId) {
          const carIdString =
            typeof testDrives[index].carId === "object"
    ...
    fetchAllData();
    // Simulating author fetch - replace with actual author fetch logic
    setAuthor({ _id: "someAuthorId" });
  }, []);
  ...
  return (
    <LayoutGroup>
      <div className="min-h-screen bg-gradient-to-br from-orange-50 to-blue-50 py-8 px-4">
        <motion.div layout className="max-w-7xl mx-auto space-y-6">
          <motion.div
          ...
          >
            <AnimatePresence mode="popLayout">
              {testDriveData
                .filter((drive) => params.id === drive.author) // Filter test drive data where params.id matches drive.author
                .map((drive, index) => {
                  const carIdString =
                    typeof drive.carId === "object"
                      ? drive.carId.$oid
                      : drive.carId;
                  ...
                  return (
                    ...
                    >
                      <div className="relative h-48">
                        <motion.img
                          src={
                            carData[carIdString]?.images?.[0]?.url ||
                            "/default-image.jpg"
                          }
                        ...
                        <motion.div className="p-4">
                          <motion.h2 className="text-lg font-semibold text-gray-900 mb-3 truncate">
                            {carData[carIdString]?.carName ||
                              "Car Name Not Available"}
                          </motion.h2>

                          {/* Display Test Drive Status */}
                          <motion.div
                            className={`mb-4 py-2 px-3 rounded-lg ${
                              getStatusStyle(drive.status).bg
                            } border-l-4 ${
                              getStatusStyle(drive.status).border
                            }`}
                          >
                            <motion.span
                              className={`flex items-center gap-2 text-sm font-medium ${
                                getStatusStyle(drive.status).text
                              }`}
                            >
                              <span
                                className={getStatusStyle(drive.status).icon}
                              >
