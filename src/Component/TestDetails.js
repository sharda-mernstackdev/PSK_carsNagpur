...
const EnhancedTestDriveDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`http://15.207.18.250:3000/api/testDrive/get/${id}`, {
          method: "GET",
          headers: {
  ...
  const handleCancel = async () => {
    try {
      const response = await fetch(`http://15.207.18.250:3000/api/testDrive/${id}/cancel`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
  ...
  const confirmDelete =async () => {
    try {
      const response = await fetch(`http://15.207.18.250:3000/api/testDrive/delete/${id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
