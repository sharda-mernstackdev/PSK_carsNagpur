...
const fetchCarsData = async (budget, selectedFilters) => {
  try {
    ...
    const response = await fetch(
      `http://15.206.194.232:3000/api/cars/cars?budgetMin=${budget[0]}&budgetMax=${budget[1]}&filters=${filterQuery}`,
      {
        method: 'GET',
        headers: {
...
function UsedCars() {
  ...
  const handleAddToCart = async (carId) => {
    try {
      ...
      const response = await fetch('http://15.206.194.232:3000/api/cart/add', {
        method: 'POST',
        credentials: 'include',
        headers: {
  ...
  function LoginModal({ isOpen, onClose, onLogin }) {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const response = await fetch("http://15.206.194.232:3000/api/users/login", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
